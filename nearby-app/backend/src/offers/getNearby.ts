...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    for (const result of queryResults) {
      if (result.Items) {
        for (const item of result.Items) {
          uniqueOffers.set(item.offerId, item as Offer);
        }
      }
    }
    ...
    const merchants = (batchGetResult.Responses?.[Tables.MERCHANTS!] as Merchant[]) || [];
    ...
    const nearbyOffers = offers
      .map((offer) => {
        ...
        if (categoryId && offer.categoryId !== categoryId) return null;
      ...
      .filter((o): o is Offer & { distance: string; merchant: Merchant } => o !== null);
