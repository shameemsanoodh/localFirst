...
async function listProducts(merchantId: string, headers: any) {
...
async function addProduct(merchantId: string, body: any, headers: any) {
...
async function updateProduct(merchantId: string, productId: string, body: any, headers: any) {
    ...
    const products: Product[] = result.Item?.products || [];
...
async function deleteProduct(merchantId: string, productId: string, headers: any) {
    const result = await dynamodb.get({
        TableName: MERCHANTS_TABLE,
        Key: { merchantId },
        ProjectionExpression: 'products',
