...
export const handler = async (event) => {
    ...
    Format as JSON: { "items": ["item1", "item2"], "description": "detailed description", "categories": ["category1"] }`;
        ...
        return response.error(`Failed to create smart broadcast: ${error.message}`, 500, 'SMART_BROADCAST_ERROR');
    }
};
async function findNearbyMerchants(lat, lng, radius, categories) {
    try {
        ...
        const nearbyMerchants = allMerchants.filter((merchant) => {
            if (!merchant.latitude || !merchant.longitude)
                return false;
            const distance = calculateDistance(lat, lng, merchant.latitude, merchant.longitude);
            const inRange = distance <= radius;
            const matchesCategory = !categories.length || categories.some(cat => merchant.categories?.includes(cat));
...
//# sourceMappingURL=createSmartBroadcast.js.map