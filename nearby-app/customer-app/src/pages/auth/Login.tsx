...
const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Block merchants from logging in to customer app
      if (response.user.role === 'merchant') {
        setError('Merchants cannot sign in here. Please use the Merchant App.')
        setLoading(false)
        return
      }
      ...
      // Block merchants in dev mode too
      if (role === 'merchant') {
        setError('Merchants cannot sign in here. Please use the Merchant App.')
        setLoading(false)
        return
      ...
      >
        ...
        {error && (
          <motion.div
            ...
            style={{ background: '#FFF4F4', border: '2px solid #FF5454' }}
          >
            <p className="text-sm font-semibold mb-2" style={{ color: '#CC2222' }}>{error}</p>
            {error.includes('Merchant') && (
              <a
                href="http://localhost:5174/login"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 text-sm font-bold mt-2 px-4 py-2 rounded-lg"
        ...
        <div className="space-y-3">
          ...
          <a href="http://localhost:5174/signup" target="_blank" rel="noopener noreferrer">
            <motion.div
              whileTap={{ scale: 0.97 }}
              className="w-full p-4 rounded-2xl flex items-center gap-3 cursor-pointer"
              style={{ background: '#F5C84218' }}
            >
              <Store size={20} style={{ color: '#1A1A1A' }} />
              <div className="flex-1">
                <p className="text-sm font-bold" style={{ color: '#1A1A1A' }}>Register your shop</p>
                <p className="text-xs" style={{ color: '#6B6B6B' }}>List your business on Nearby</p>
              </div>
              <ArrowRight size={16} style={{ color: '#9A9895' }} />
            </motion.div>
          </a>
